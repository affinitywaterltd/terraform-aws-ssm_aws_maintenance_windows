...
resource "aws_ssm_maintenance_window_task" "default_task_start_stopped_instances" {
  ...
  targets {
    key = "WindowTargetIds"
    values = [element(
      aws_ssm_maintenance_window_target.default_pre.*.id,
      count.index,
    )]
  }
}
  ...
  task_invocation_parameters {
    run_command_parameters {
      ...
      parameter {
        ...
        values = [var.weeks > 1 ? "${(((count.index + 1) - local.patches_days_old_week_offset) * 7) + lookup(local.days_of_week, var.day, 0)}" : "${((var.week - local.patches_days_old_week_offset) * 7) + lookup(local.days_of_week, var.day, 0)}"]
      }
    }
  }
}
