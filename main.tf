...
#
#
#      Instance Power On Task
#
#
#
resource "aws_ssm_maintenance_window_target" "power_up" {
  count         = "${var.weeks}"
  window_id     = "${element(aws_ssm_maintenance_window.default.*.id, count.index)}"
...
resource "aws_ssm_maintenance_window_task" "power_up_task_run_start" {
  count            = "${var.weeks}"
  window_id        = "${element(aws_ssm_maintenance_window.default.*.id, count.index)}"
  name             = "power_up_task_run_start"
  description      = "Power up instances for Maintenance Window"
  task_type        = "RUN_COMMAND"
  task_arn         = "AWS-ConfigureAWSPackage"
  priority         = 5
  service_role_arn = "${var.role}"
  ...
  task_parameters {
    ...
    values = ["${var.weeks > 1 ? "${var.type}_week-${count.index+1}_${var.day}_${var.hour}00/${var.account}-${var.environment}" : "${var.type}_week-${var.week}_${var.day}_${var.hour}00/${var.account}-${var.environment}" }"]
  }
}
